...
description: "Keep a reviewable decision trail for long-running or unattended work: a TSV log with one row per decision (what, why, evidence, result). Local by default; commit it when a reviewer needs the trail to trust the result. Use for $supastack:show-me-your-work, autonomous or multi-phase runs, or work a human reviews after stepping away."
---
...
At the end of the run, before handing back, check the log told the truth. Read this run's workspace-matching Codex session under `${CODEX_HOME:-$HOME/.codex}/sessions/` or `${CODEX_HOME:-$HOME/.codex}/archived_sessions/`. Don't search sessions from unrelated workspaces; that reads unrelated private chats. Walk the log against what actually happened:

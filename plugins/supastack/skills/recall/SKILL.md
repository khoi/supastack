...
Codex sessions live under `${CODEX_HOME:-$HOME/.codex}/sessions/` and `${CODEX_HOME:-$HOME/.codex}/archived_sessions/`. Match records by their workspace `cwd` and session metadata before reading them. Every JSONL line is one session event.

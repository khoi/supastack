---
name: architect
description: "Sketch types, signatures, and module structure before code, then stay in the loop while implementation fills in. Use for $supastack:architect, 'architect this', 'design this', or non-trivial work where jumping to code would lock in the wrong shape."
---
...
Call `update_plan` with one entry per phase before starting. Autonomous mode without checkpoints needs the list to show phase position and keep phases from silently disappearing.
...
Use the selector list in `architect_runners` from `${CODEX_HOME:-$HOME/.codex}/supastack/models.toml` (defaults `gpt-5.6-sol` at `max`, `gpt-5.6-sol` at `max`, `gpt-5.6-luna` at `xhigh`, `gpt-5.6-terra` at `xhigh`). Pass each selector's `model` and `reasoning_effort` as separate native Codex overrides. A legacy string is a model-only override.
...
Opt in to a checkpoint when the invoker explicitly asks: "$supastack:architect with checkpoint," "stop and show me before implementing," or similar. Then surface the synthesized design and pause for sign-off.

...
1. Locate the prior trail. A workspace-matching Codex session under `${CODEX_HOME:-$HOME/.codex}/sessions/` or `${CODEX_HOME:-$HOME/.codex}/archived_sessions/` (do not search unrelated workspaces; that crosses workspace boundaries and reads private chats from unrelated projects), a Codex agent URL, or a pushed branch. Read the metadata overview and last messages first, then scan back for the decision points. Parse a long session in a subagent and keep the reduced timeline in the main thread (the **principle-guard-the-context-window** skill).

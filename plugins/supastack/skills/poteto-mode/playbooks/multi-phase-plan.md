...
3. Explore with native Codex subagents using the installed `supastack_worker` role, or `worker`/`default` as a fallback, and a model per the Subagents section when overrides are available (the **guard-the-context-window** principle skill). Each returns file pointers, conventions, test commands, and entry points. No inlined dumps.

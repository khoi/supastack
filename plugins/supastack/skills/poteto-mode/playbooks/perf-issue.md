...
3. Plan the fix from the trace. If it crosses a function boundary, `architect` first. Delegate implementation to a subagent using your configured `perf_issue` selector (default `gpt-5.6-sol` at `max`). Pass `model` and `reasoning_effort` as separate native Codex overrides; review the diff. Capture a post-fix trace.
